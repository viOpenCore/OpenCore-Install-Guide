...
| Note 1 | iGPU của Sandy Bridge chỉ được hỗ trợ chính thức đến macOS 10.13 |
| Note 2 | Hầu hết bo mạch chủ Sandy Bridge không hỗ trợ UEFI |
...
| :--- | :--- |
| **[SSDT-PM](https://github.com/Piker-Alpha/ssdtPRGen.sh)** | Needed for proper CPU power management, you will need to run Pike's ssdtPRGen.sh script to generate this file. This will be run in [post install](https://viopencore.github.io/OpenCore-Post-Install/). |
...
| **[SSDT-IMEI](https://viopencore.github.io/Getting-Started-With-ACPI/)** | Needed to add a missing IMEI device on Sandy Bridge CPU with 7 series motherboards |
...
This blocks certain ACPI tables from loading, for us we really care about this. Main reason is that Apple's XCPM does not support Sandy Bridge all too well and can cause AppleIntelCPUPowerManagement panics on boot. To avoid this we make our own PM SSDT in [Post-Install](https://viopencore.github.io/OpenCore-Post-Install/) and drop the old tables(Note that this is only temporary until we've made our SSDT-PM, we'll re-enable these tables later):
